...
class MetamaskIdentityProvider extends IdentityProvider {
  ...
  async getId (options = {}) {
    ...
    if (!wallet) {
      throw new Error(`No address detected. Make sure Metamask is unlocked and your defaultAccount is set in web3.`)
    }
  ...
  static async verifyIdentity (identity) {
    ...
    const signerAddress = await verifyMessage(identity.publicKey + identity.signatures.id, identity.signatures.publicKey)
