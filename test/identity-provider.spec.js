...
describe('Identity Provider', function () {
  ...
  describe('create an identity', () => {
    describe('creates a default keystore if none is passed', async () => {
      const id = 'A'
  ...
  describe('create identity from existing keys', () => {
    const source = path.resolve(signingKeysPath, 'existing')
    ...
    before(async () => {
      await fs.copy(fixturesPath, signingKeysPath)
